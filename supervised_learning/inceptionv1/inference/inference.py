...
def inference(model, testloader, device):
    ...
    dataiter = iter(testloader) # iterator to fetch batches of data one at a time
    images, labels = next(dataiter) # used to get next batch
    ...
    for i in range(9):
        ax = axes[i // 3, i % 3] 
        # i // 3 calculats the row (for i = (0, 1, 2) will be 0)
        # i % 3 calcualates the column (for i = (0, 1, 2) will be (0, 1, 2))
        img = images[i].cpu().numpy().transpose((1, 2, 0)) # images typically has (C, H, W) but matplotlib expects (H, W, C) 
